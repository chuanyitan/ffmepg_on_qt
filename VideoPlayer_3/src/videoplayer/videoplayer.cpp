...
void VideoPlayer::run()
{
    ...
    char *file_path = "E:\\2\\VideoPlayer_1\\Forrest.mp4";//通过传参获取
    ...
    static struct SwsContext *img_convert_ctx;  //？？是什么结构体
    ...
    if (SDL_Init(SDL_INIT_AUDIO)) {  //初始化SDL
        fprintf(stderr,"Could not initialize SDL - %s. \n", SDL_GetError());
        exit(1);
    }

    //Allocate an AVFormatContext.
    pFormatCtx = avformat_alloc_context();//上下文结构体

    if (avformat_open_input(&pFormatCtx, file_path, NULL, NULL) != 0) {///打开一个文件
    ...
    qDebug()<<pFormatCtx->streams[0]->duration ;
    qDebug()<<pFormatCtx->streams[0]->duration * av_q2d(pFormatCtx->streams[0]->time_base);
    //qDebug()<<pFormatCtx->streams[0]->time_base;
    qDebug()<<av_q2d(pFormatCtx->streams[0]->time_base);
    ...
    mVideoState.aCodecCtx = aCodecCtx;   //自己定义的结构体
    ...
    wanted_spec.freq = aCodecCtx->sample_rate; //设置播放采样率
    ...
    mVideoState.video_st = pFormatCtx->streams[videoStream];//AVStream *video_st;  VideoState
    ...
    int64_t start_time = av_gettime();  ////////////////////////开始进行同步  av_gettime()系统当前的时间
    int64_t pts = 0; //当前视频的pts

    while (1)
    {
        if (av_read_frame(pFormatCtx, packet) < 0)//从输入文件中读入一帧压缩数据
        {
            break; //这里认为视频读取完了
        }

        int64_t realTime = av_gettime() - start_time; //主时钟时间
       // qDebug()<<"realtime:"<<realTime<<"av_gettime:"<<av_gettime()<<"start time:"<<start_time;
        ...
        if (packet->stream_index == videoStream)
        {
            ...
            pts *= 1000000 * av_q2d(mVideoState.video_st->time_base);//根据pts来计算一桢在整个视频中的时间位置
          //  qDebug()<<"pts1:"<<pts;
            pts = synchronize_video(&mVideoState, pFrame, pts);//???同步？
