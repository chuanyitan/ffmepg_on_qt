...
{
    char *file_path = "E:\\2\\VideoPlayer_1\\Forrest.mp4";
    ...
    img_convert_ctx = sws_getContext(pCodecCtx->width, pCodecCtx->height,
            pCodecCtx->pix_fmt, pCodecCtx->width, pCodecCtx->height,//把原来的格式数据转化成RGB的数据
            PIX_FMT_RGB24, SWS_BICUBIC, NULL, NULL, NULL);

    //avpicture_get_size() 获得PIX_FMT_RGB24像素格式，图片大小为这么多的字节数
    ...
    while (1)
    {
        ...
        if (packet->stream_index == videoStream) {
            ...
            if (got_picture) {
                ...
                if (index > 200) return 0; //这里我们就保存50张图片
            }
        }
